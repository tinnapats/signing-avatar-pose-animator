...
export const HAND_BONE_CONNECTIONS = [
    [0, 1], [1, 2], [2, 3], [3, 4],
    [0, 5], [5, 6], [6, 7], [7, 8],
...
const handPartNames = HAND_PART_NAMES.left.concat(HAND_PART_NAMES.right);
const handPartLookup = {};
Object.keys(HAND_PART_NAMES).forEach(side => {
    HAND_PART_NAMES[side].forEach((name, landmarkIndex) => {
        handPartLookup[name] = { side, landmarkIndex };
    });
});

const handRestOffsets = [
    { forward: 0.0, spread: 0.0 },
    { forward: 0.26, spread: 1.45 },
    { forward: 0.58, spread: 1.78 },
    { forward: 0.92, spread: 2.02 },
    { forward: 1.24, spread: 2.18 },
    { forward: 0.54, spread: 0.98 },
    { forward: 1.02, spread: 1.02 },
    { forward: 1.48, spread: 1.06 },
    { forward: 1.92, spread: 1.10 },
    { forward: 0.68, spread: 0.34 },
    { forward: 1.26, spread: 0.36 },
    { forward: 1.84, spread: 0.38 },
    { forward: 2.40, spread: 0.40 },
    { forward: 0.64, spread: -0.26 },
    { forward: 1.18, spread: -0.28 },
    { forward: 1.72, spread: -0.30 },
    { forward: 2.20, spread: -0.32 },
    { forward: 0.50, spread: -0.95 },
    { forward: 0.92, spread: -1.04 },
...
export const allPartNames = posePartNames.concat(facePartNames, handPartNames);

export function buildDefaultHandKeypoints(side, wristPosition, elbowPosition) {
    const names = HAND_PART_NAMES[side] || [];
    if (!names.length) {
        return {};
    }

    const wrist = new paperLib.Point(wristPosition.x, wristPosition.y);
    const elbow = new paperLib.Point(elbowPosition.x, elbowPosition.y);
    let forearm = wrist.subtract(elbow);
    if (!forearm.length) {
        forearm = new paperLib.Point(side === 'left' ? 1 : -1, 1);
    }
    const forward = forearm.normalize();
    const spread = forward.clone();
    spread.angle += side === 'left' ? -90 : 90;
    const forearmLen = Math.max(forearm.length, 24);
    const forwardUnit = forearmLen * 0.30;
    const spreadUnit = forearmLen * 0.17;

    const out = {};
    names.forEach((name, idx) => {
        const spec = handRestOffsets[idx] || handRestOffsets[0];
        const position = wrist
            .add(forward.multiply(spec.forward * forwardUnit))
            .add(spread.multiply(spec.spread * spreadUnit));
        out[name] = {
            position: position,
            name: name,
        };
...
export class Bone {
    ...
    getPointTransform(p) {
        ...
        return {
            transform: new paperLib.Point(dirProjD, dirProjN),
    ...
    transform(trans) {
        ...
        let scale = this.skeleton.currentBodyScale;
        if (this.type === 'face') {
            scale = this.skeleton.currentFaceScale;
        } else if (this.type === 'hand') {
            scale = this.skeleton.currentHandScale;
        }
...
function getPartFromPose(pose, name) {
    ...
    return {
        position: new paperLib.Point(part.position.x, part.position.y),
        score: part.score,
    }
}

function getPartFromHands(hands, name) {
    let lookup = handPartLookup[name];
    if (!lookup || !hands || !hands[lookup.side] || !hands[lookup.side].keypoints) {
        return null;
    }
    let part = hands[lookup.side].keypoints[lookup.landmarkIndex];
    if (!part || !part.position) {
        return null;
    }
    return {
...
function getKeypointFromFaceFrame(face, i) {
    ...
    return new paperLib.Point(face.positions[i * 2], face.positions[i * 2 + 1]);
}
...
export class Skeleton {
    constructor(scope) {
        ...
        let leftHandKeypoints = buildDefaultHandKeypoints('left', leftWrist.position, leftElbow.position);
        let rightHandKeypoints = buildDefaultHandKeypoints('right', rightWrist.position, rightElbow.position);
        ...
        this.leftHandBones = HAND_BONE_CONNECTIONS.map(([fromIdx, toIdx]) => {
            return new Bone().set(leftHandKeypoints[HAND_PART_NAMES.left[fromIdx]], leftHandKeypoints[HAND_PART_NAMES.left[toIdx]], this, 'hand');
        });
        this.rightHandBones = HAND_BONE_CONNECTIONS.map(([fromIdx, toIdx]) => {
            return new Bone().set(rightHandKeypoints[HAND_PART_NAMES.right[fromIdx]], rightHandKeypoints[HAND_PART_NAMES.right[toIdx]], this, 'hand');
        });
        ...
        this.handBones = this.leftHandBones.concat(this.rightHandBones);
        this.bones = this.faceBones.concat(this.bodyBones, this.handBones);
        this.secondaryBones = [];
        this.parts = {};
        this.bodyLen0 = this.getTotalBoneLength(this.bodyBones);
        this.faceLen0 = this.getTotalBoneLength(this.faceBones);
        this.handLen0 = this.getTotalBoneLength(this.handBones);
        this.currentBodyScale = 1;
        this.currentFaceScale = 1;
        this.currentHandScale = 1;
        ...
        this.boneGroups = {
            ...
            'leftHand': this.leftHandBones,
            'rightHand': this.rightHandBones,
            'face': this.faceBones,
        };
    ...
    update(pose, face, hands) {
        ...
        const handScale = this.handLen0 > 0 ? this.getTotalBoneLength(this.handBones) / this.handLen0 : this.currentBodyScale;
        this.currentHandScale = handScale * 1.08;
        this.isValid = true;
    }
    ...
    updateHandParts(hands) {
        if (!this.parts['leftWrist'] || !this.parts['leftElbow'] || !this.parts['rightWrist'] || !this.parts['rightElbow']) {
            return;
        }
        ...
        ['left', 'right'].forEach(side => {
            ...
            const partNames = HAND_PART_NAMES[side];
            const fallbackParts = buildDefaultHandKeypoints(
                side,
                this.parts[wristName].position,
                this.parts[elbowName].position
            );
            const rawParts = {};
            partNames.forEach(partName => {
                rawParts[partName] = getPartFromHands(hands, partName);
            });
            ...
            partNames.forEach((partName, landmarkIndex) => {
                ...
                const fallback = fallbackParts[partName];
    ...
    findBoneGroup(point) {
        ...
        if (typeof selectedGroups.flat === 'function') {
            return selectedGroups.flat();
        }
        return [].concat.apply([], selectedGroups);
    }
    ...
    static getCurrentPosition(segment) {
        let position = new paperLib.Point();
