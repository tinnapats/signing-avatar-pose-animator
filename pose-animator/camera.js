...
function applyWaistCameraFraming(pose) {
  ...
  const anchor = getHipAnchor(pose) || { x: videoWidth * 0.5, y: videoHeight * 0.72 };
  const targetX = videoWidth * WAIST_CAMERA_TARGET_X;
  const targetY = videoHeight * WAIST_CAMERA_TARGET_HIP_Y;
...
function detectPoseInRealTime(video) {
  ...
  async function poseDetectionFrame() {
    ...
    if (poses.length >= 1 && illustration) {
      ...
      applyWaistCameraFraming(poses[0]);
