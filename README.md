...
## ฐานข้อมูลคลิปในเครื่อง

เว็บใช้ `C:\pro1end\SLclean.sqlite3` เป็นค่าเริ่มต้น อ่านข้อมูลคลิปจากตาราง `clips` โดยตรงแบบ read-only ไม่แตกไฟล์ CSV ลงดิสก์ และแคชข้อมูลคลิปที่คลายการบีบอัดล่าสุดไม่เกิน 8 คลิป การค้นวลียาวที่สุดและป๊อปอัปทำงานเหมือนเดิม

เปลี่ยนแหล่งข้อมูลด้วย `--data-dir "ตำแหน่งฐานข้อมูล.sqlite3"` หรือระบุโฟลเดอร์ CSV เพื่อใช้รูปแบบเดิมได้ ฐานข้อมูลต้องใช้รูปแบบ `slclean-csv-zlib-v1` และเป็นสำเนาตอนนำเข้า ไม่ซิงก์ CSV อัตโนมัติ ตรวจแหล่งข้อมูลที่ใช้งานได้จาก `/api/health` ช่อง `storage` และ `dataDir`

สร้างฐานข้อมูลจาก CSV ด้วยคำสั่ง:

```powershell
.\.venv\Scripts\python.exe .\import_slclean_sqlite.py --source "C:\pro1end\SLclean" --output "C:\pro1end\SLclean.sqlite3"
```

จากนั้นรัน `run_pose_animator_server.cmd --data-dir "C:\pro1end\SLclean.sqlite3"` หรือระบุพาธที่ใช้ในเครื่องของคุณ ดูรายละเอียดตารางและตัวอย่างอ่านข้อมูลใน `SLclean_database.md` ไม่รวมฐานข้อมูลส่วนตัวไว้ใน GitHub
